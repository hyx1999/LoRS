...
def parse_args():
    ...
    parser.add_argument("--peft_method", type=str, default="lors", 
        choices=["lora", "lors", "sqft", "sqft-gc", "spp", "spp-gc", "none"]
    )
    parser.add_argument("--lors_samples", type=int, default=128)
    ...
    if args.profile:
        ...
        assert args.max_train_steps > PROFILE_WARMUP_STEPS
    return args
...
def main():
    ...
    assert args.dataset_name == "alpaca"
    ...
    if args.profile:
        collate_fn=transformers.DataCollatorForSeq2Seq(
            tokenizer, max_length=args.cutoff_len, return_tensors="pt", padding="max_length"
        )
    else:
        collate_fn=transformers.DataCollatorForSeq2Seq(
    ...
    if args.profile:
        with open(os.path.join(args.output_dir, "profile.json"), "w") as f:
            memory = f"Max Allocated: {torch.cuda.max_memory_allocated() / 1024 ** 2:.2f} MB"
            json.dump({
                "batch_size": args.per_device_train_batch_size,
                "cutoff_len": args.cutoff_len,
                "rank": args.lora_rank,
                "memory": memory,
                "time": time_usage / (args.max_train_steps - PROFILE_WARMUP_STEPS),
            }, f, indent=4)
        exit(0)
    ...
    with torch.no_grad():
        ...
        ppl = eval_ppl(args, base_model, tokenizer, device)
        if accelerator.is_main_process:
            print("wikitext2 ppl: {}".format(ppl))
